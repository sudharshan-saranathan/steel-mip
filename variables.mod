...
var coking_coal_in{t in T}      >= 0,  <= 1.0*dem[t];  # X[2] (Fix3: valid bound for CCS-term McCormick)
...
var sinter_lime_in{t in T}      >= 0,  <= 0.1*dem[t];  # X[10] (Fix3 bound)
...
var bf_lime_in{t in T}          >= 0,  <= 0.1*dem[t];  # X[20] (Fix3 bound)
...
var bf_coalpci_in{t in T}       >= 0,  <= 0.25*dem[t];  # X[28] (Fix3 bound)
...
var bof_lime_in{t in T}         >= 0,  <= 0.1*dem[t];  # X[35] (Fix3 bound)
...
# ----------------- Coal DRI -----------------
var coaldri_output{t in T}      >= 0,  <= (1-n7_phi_eaf)*dem[t];  # X[40] (Fix3 bound)
...
# ------------------- NG DRI -----------------
var ngdri_output{t in T}        >= 0,  <= (1-n7_phi_eaf)*dem[t];  # X[45] (Fix3 bound)
...
# Fix 3: the capture fractions fc_bf / fc_cdri / fc_ngdri are NO LONGER
# optimization variables. The captured CO2 amounts (ccs_bf/ccs_cdri/ccs_ngdri,
# declared above) are now the decisions, bounded by the technical capture limit
# (see modules/q_carbon_capture.mod). This removes the bilinear product
# (capturable_base x fc_*) that previously made the CCS block nonconvex. The
# realized capture fraction is recovered post-solve for reporting (yreport.mod).
# capbase_* are auxiliary variables holding the (linear) capturable CO2 base.
...
# Fix 2: f_cdri / f_ngdri are no longer optimization variables. The coal/NG/H2
# route *outputs* (coaldri_output, ngdri_output, h2dri_output) are the decisions,
# linked by dri_route_split. Route fractions are reported post-solve (see report.mod).
