...
# ===================================================================
# Fix 3 (preferred): the captured CO2 amount ccs_* is now the decision
# variable, bounded above by the technical capture limit
#     ccs_X[t] <= n10_ccs_eta * fc_max * capbase_X[t],
# where capbase_X[t] is the (linear) capturable CO2 base for route X and
# fc_max is the maximum capture penetration (formerly the upper bound on
# the fc_* variables). This eliminates the previous bilinear product
# (capbase_X * fc_X), so the CCS block is fully linear. The optimizer chooses
# how much to capture (driven by capture cost and the emissions target); the
# realized capture fraction is recovered post-solve in yreport.mod.
# ===================================================================
param fc_max := 0.9;   # max capture penetration fraction (was the fc_* upper bound)

# -----------------------------
# Capturable CO2 base per route  (linear in flows; same coefficients as the
# original eq84/85/86 capture expressions)
# -----------------------------
...
# -----------------------------
# Valid upper bounds on the capturable base (from the Fix-1/Fix-3 variable
# bounds in variables.mod). Used only to size the big-M in the capture
# phase-in switches (modules/t_additional_constraints.mod). Because
# |ccs_X[t] - ccs_X[t-1]| <= max(ccs_X) = n10_ccs_eta*fc_max*cap_ub_X[t],
# these are valid (non-binding) big-M values.
# -----------------------------
