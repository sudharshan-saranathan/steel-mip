...
for {t in T} {
    printf "\n%4d %8.0f/%5.2f %8.0f/%5.2f %8.0f/%5.2f %8.0f/%5.2f %10.0f/%5.2f %10.0f",
        ...
        steel_eaf[t] * share_coal[t], share_coal[t] * f_eaf[t],                            # Coal DRI
        steel_eaf[t] * share_ng[t] , share_ng[t] * f_eaf[t],                             # NG DRI
        steel_eaf[t] * (1 - share_coal[t] - share_ng[t]), (1 - share_coal[t] - share_ng[t]) * f_eaf[t],            # H2 DRI
        steel_scrap_eaf[t], 1 - f_bof[t] - share_coal[t] * f_eaf[t] - share_ng[t] * f_eaf[t] - (1 - share_coal[t] - share_ng[t]) * f_eaf[t],            # Scrap-EAF
        total_steel[t];                                          # Total
}
...
for {t in T} {
    printf "\n%4d %10.0f/%5.2f %10.0f/%5.2f %10.0f/%5.2f %20.0f %12.3f",
        ...
        ccs_cdri[t],
        if total_ccs[t] > 0 then ccs_cdri[t] / (steel_eaf[t] * share_coal[t]) else 0,

        # NG–DRI CCS and share
        ccs_ngdri[t],
        if total_ccs[t] > 0 then ccs_ngdri[t] / (steel_eaf[t] * share_ng[t])  else 0,
...
for {t in T} {
    printf "\n%4d %12.2f %15.2f %15.2f %15.2f %15.2f",
        ...
        # Coal DRI-EAF
        if steel_eaf[t]*share_coal[t] > 0 then
           (carbon_tax*scope1_cdri[t] + n10_ccs_cost[t]*ccs_cdri[t]
           + cost_coaldri[t] + share_coal[t]*cost_eaf[t] + cost_pellet_coaldri[t]
           + (labor_cost + maintenance_cost + other_opex)*(steel_eaf[t]*share_coal[t])
           - (wasteheat_eaf[t] * 0.9 * 277.78 * n9_eta * n9_whr[t] *ng_credit_power 
           - wasteheat_eaf[t] * 0.9 * 277.78 * n9_eta * n9_whr[t] * n9_whr_capex
           - wasteheat_eaf[t] * 0.9 * 277.78 * n9_eta * n9_whr[t] * n9_whr_opex) * share_coal[t])/ (steel_eaf[t]*share_coal[t])
        else 0,

        # NG DRI-EAF
        if steel_eaf[t]*share_ng[t] > 0 then
           (carbon_tax*scope1_ngdri[t] + n10_ccs_cost[t]*ccs_ngdri[t]
           + cost_ngdri[t] + share_ng[t]*cost_eaf[t] + cost_pellet_ngdri[t]
           + (labor_cost + maintenance_cost + other_opex)*(steel_eaf[t]*share_ng[t])
           - (wasteheat_eaf[t] * 0.9 * 277.78 * n9_eta * n9_whr[t] *ng_credit_power 
           - wasteheat_eaf[t] * 0.9 * 277.78 * n9_eta * n9_whr[t] * n9_whr_capex
           - wasteheat_eaf[t] * 0.9 * 277.78 * n9_eta * n9_whr[t] * n9_whr_opex) * share_ng[t])
           / (steel_eaf[t]*share_ng[t])
        else 0,

        # H2 DRI-EAF
        if t >= ng_h2_start_year && steel_eaf[t]*(1-share_coal[t]-share_ng[t]) > 0 then
           (carbon_tax*scope1_h2dri[t] + cost_h2dri[t]
           + (1-share_coal[t]-share_ng[t])*cost_eaf[t] + cost_pellet_h2dri[t]
           + (labor_cost + maintenance_cost + other_opex)*(steel_eaf[t]*(1-share_coal[t]-share_ng[t]))
           - (wasteheat_eaf[t] * 0.9 * 277.78 * n9_eta * n9_whr[t] *ng_credit_power 
           - wasteheat_eaf[t] * 0.9 * 277.78 * n9_eta * n9_whr[t] * n9_whr_capex
           - wasteheat_eaf[t] * 0.9 * 277.78 * n9_eta * n9_whr[t] * n9_whr_opex) * (1- share_coal[t] - share_ng[t]))
           / (steel_eaf[t]*(1-share_coal[t]-share_ng[t]))
        else 0,
...
for {t in T} {
    ...
    printf "\n%4d %12.3f %15.3f %15.3f %15.3f %15.3f",
        ...
        if steel_eaf[t]*share_coal[t] > 0 then
            (scope1_cdri[t]
             + n9_grid_ef[t] * (coaldri_power_in[t] + share_coal[t]*eaf_power_in[t] + pellets_power_coaldri[t])
             - ccs_cdri[t])
            / (steel_eaf[t]*share_coal[t])
        else 0,

        # ------------------ NG DRI-EAF ------------------
        if steel_eaf[t]*share_ng[t] > 0 then
            (scope1_ngdri[t]
             + n9_grid_ef[t] * (ngdri_power_in[t] + share_ng[t]*eaf_power_in[t] + pellets_power_ngdri[t])
             - ccs_ngdri[t])
            / (steel_eaf[t]*share_ng[t])
        else 0,
        ...
        # ------------------ H2 DRI-EAF ------------------
        if t >= ng_h2_start_year && steel_eaf[t]*(1-share_coal[t]-share_ng[t]) > 0 then
            (( eaf_coal_in[t] * (1- share_coal[t]-share_ng[t]) * 0.110*24+ eaf_lime_in[t] * (1- share_coal[t]-share_ng[t]) * 0.44 + eaf_electrode_in[t] *  (1- share_coal[t]-share_ng[t]) * 6)
             + n9_grid_ef[t] * ((1-share_coal[t]-share_ng[t])*eaf_power_in[t] + pellets_power_h2dri[t] + h2dri_power_in[t]))
            / (steel_eaf[t]*(1-share_coal[t]-share_ng[t]))
        else 0,
